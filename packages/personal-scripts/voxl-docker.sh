...
ARGUMENTS:
  ...
  -c <num> : (optional) Limit CPU cores used by the container, e.g. -c 4
  -m <mem> : (optional) Limit memory used by the container, e.g. -m 4g
...
while getopts 'phd:i:le:c:m:' opt
do
	case $opt in
	h)
		printUsage
	...
	l)
		...
		;;
	c)
		CPU_LIMIT="--cpus=$OPTARG"
		;;
	m)
		MEM_LIMIT="--memory=$OPTARG"
		;;
	*)
		printUsage
		;;
...
# Mount Claude Code config to a separate directory (not the working dir)
# This avoids polluting the project directory with Claude config files
CLAUDE_MOUNTS=""
if [ -d "$HOME/.claude" ]; then
  CLAUDE_MOUNTS="-v $HOME/.claude:/opt/claude-home/.claude:rw"
  [ -f "$HOME/.claude.json" ] && CLAUDE_MOUNTS+=" -v $HOME/.claude.json:/opt/claude-home/.claude.json:rw"
fi
